...
class TranscriptionService(EventEmitter):
    def __init__(self):
        ...
        self.is_connected = False  # Initialize connection flag
    ...
    async def connect(self):
        try:
            self.deepgram_live = await self.client.listen.v("1").start(LiveOptions(
                model="nova-2",
                language="en-US",
                encoding="mulaw",
                sample_rate=8000,
            ...
            logger.exception("Error during Deepgram connection:")
    ...
    async def handle_utterance_end(self, self_obj, utterance_end):
        try:
            ...
            logger.exception("Error while handling utterance end:")
        ...
        try:
            ...
            if result.is_final and text.strip():
                ...
                if result.speech_final:
                    self.speech_final = True
                    await self.emit('transcription', self.final_result.strip())
                    self.final_result = ''
                else:
                    self.speech_final = False
            elif text.strip():
            ...
            logger.exception("Error while handling transcription:")
    ...
    async def handle_warning(self, self_obj, warning):
        logger.info(f'Deepgram warning: {warning}')
